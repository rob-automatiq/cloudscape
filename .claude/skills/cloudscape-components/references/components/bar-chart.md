...
Generated from `@cloudscape-design/components@3.0.1382` `bar-chart/interfaces.d.ts`. Descriptions are the library's own JSDoc (the same text as the API tab on cloudscape.design).
...
Supporting types from `BarChartProps` (verbatim):
