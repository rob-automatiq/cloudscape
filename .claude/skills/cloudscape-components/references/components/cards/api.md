...
Generated from `@cloudscape-design/components@3.0.1382` `cards/interfaces.d.ts`. Descriptions are the library's own JSDoc (the same text as the API tab on cloudscape.design).

## Properties

- **`ariaLabels?`**: `CardsProps.AriaLabels<T>` _(@i18n)_
...
- **`cardDefinition`**: `CardsProps.CardDefinition<T>`
  ...
  * `header` ((item) => ReactNode) - Responsible for displaying the card header. You receive the current item as an argument.
      Use `fontSize="inherit"` on [link](../link/guidelines.md) components inside card header.
...
- **`pagination?`**: `React.ReactNode`
  Use this slot to add the [pagination component](../pagination/guidelines.md) to the component.
- **`preferences?`**: `React.ReactNode`
  Use this slot to add [collection preferences](../collection-preferences/guidelines.md) to the component.
...
Supporting types from `CardsProps` (verbatim):
