...
> Source: https://cloudscape.design/components/modal/ (fetched 2026-09-24)
...
A user interface element subordinate to an application's main window. It prevents interaction with the main page content, but keeps it visible with the modal as a child window in front of it.
 ...
 [Get design library](../../get-started/for-designers/design-resources.md) [Browse code](https://github.com/cloudscape-design/components/tree/main/src/modal)
...
If you want to place multiple buttons in the `footer` slot, use the [space between](../space-between/guidelines.md) component to add horizontal spacing between them.

#### State management

The Modal component is controlled. Set the `visible` property and the `onDismiss` listener to control its visibility. Learn more about the [state management](../../get-started/dev-guides/state-management.md) of Cloudscape components.
...
- Use a [flash message](../flashbar/guidelines.md)   to confirm the result of committing modal data.
- Use a modal primarily to confirm or cancel a choice. For example, [deleting a resource](../../patterns/resource-management/delete.md)  .
...
- Do not chain together a sequence of modals. Instead of using a sequence of modals with multiple steps over multiple pages, use the [multipage create flow](../../patterns/resource-management/create/multi-page-create.md)  .
...
- #### Content
  ...
  - [Alert](../alert/guidelines.md)     and description to inform the consequences of the user actions, for example, in [delete](../../patterns/resource-management/delete.md)     pattern to provide details of a delete action and in [communicating unsaved changes](../../patterns/general/unsaved-changes.md)     to communicate unsaved changes.
  - [Input fields](../input/guidelines.md)     and simple [selects](../select/guidelines.md)     to create resource, for example, in [create resource flow](../../patterns/resource-management/create.md)    .
  - [Tiles](../tiles/guidelines.md)     and description to facilitate comparison, for example, in [density settings](../../patterns/general/density-settings.md)     to compare content density modes and in [split panel](../split-panel/guidelines.md)     to compare display modes.
  - [Checkboxes](../checkbox/guidelines.md)     , [radio groups](../radio-group/guidelines.md)     , and [toggles](../toggle/guidelines.md)     to change preference settings, for example, in [collection preferences](../collection-preferences/guidelines.md)     for table and cards.
- #### Dismiss button
...
- #### Position
  ...
  Controls the vertical positioning of the modal. The default is **center**   . Use value top to anchor the modal at a fixed distance from the top.
