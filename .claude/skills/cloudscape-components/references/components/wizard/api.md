...
Generated from `@cloudscape-design/components@3.0.1382` `wizard/interfaces.d.ts`. Descriptions are the library's own JSDoc (the same text as the API tab on cloudscape.design).
...
- **`i18nStrings?`**: `WizardProps.I18nStrings` _(@i18n)_
...
- **`steps`**: `ReadonlyArray<WizardProps.Step>`
  ...
  - `info` (ReactNode) - (Optional) Area for a page level info link that's displayed in the form header.
     The page level info link should trigger the default help panel content for the step. Use the [link component](../link/guidelines.md) to display the link.
...
Supporting types from `WizardProps` (verbatim):
