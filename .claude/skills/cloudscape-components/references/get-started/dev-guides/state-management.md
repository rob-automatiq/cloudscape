...
> Source: https://cloudscape.design/get-started/dev-guides/state-management/ (fetched 2026-09-24)

How to manage the state of Cloudscape components.

## Managing the state of a component

Our components expose properties and events that allow you to configure their appearance and behavior. Some properties represent the main information that your end users see and can interact with, such as the `value` of an [input](../../components/input/guidelines.md) component or the `selectedOption` in a [select](../../components/select/guidelines.md) component. These properties represent the state of a component. This article explains how to control those properties by introducing the concept of controllability, which is widely used in the React ecosystem.

In general, a React component can be [controlled](state-management.md) , [uncontrolled](state-management.md) , or [controllable](state-management.md) . The rest of the article explains these concepts. Our components are always either controlled (for example: input, select) or controllable (for example: [tabs](../../components/tabs/guidelines.md) ). For more information about a component's controllability, read the development guidelines for the component.
