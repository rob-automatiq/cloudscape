...
> Source: https://cloudscape.design/patterns/general/density-settings/ (fetched 2026-09-24)
...
Use a [side navigation link](../../components/side-navigation/guidelines.md) to trigger a modal. Place the link as last item in your side navigation and separate it from the other links with a divider. If your service already has a settings area include the content density selection there.
...
import { comfortableImage, compactImage } from './common';
import './index.scss';

export const comfortableModeImage = comfortableImage;

export const compactModeImage = compactImage;

interface DensitySettingsBuilder {
  description: string;
}
export default function DensitySettingsBuilder({ description }: DensitySettingsBuilder) {
  return function Component() {
    const [value, setValue] = useState<string>('item1');
    return (
      <FormField description={description} label="Content density">
        <Tiles
          value={value}
...
- Customize your view experience   You can now customize the level of content density for CloudFront console density settings. Let us know what you think  .   Open density settings
...
We recommend communicating the release of the compact mode to your users, for example by using a [flash message](../../components/flashbar/guidelines.md) . Include information about compact mode release, and an [action button](../../components/button/guidelines.md) to choose the preferred density level via density settings modal.
...
- Follow the accessibility guidelines for [tiles](../../components/tiles/guidelines.md)  .
...
[View Documentation](../../foundation/visual-foundation/content-density.md)
