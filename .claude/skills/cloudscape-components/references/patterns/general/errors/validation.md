...
> Source: https://cloudscape.design/patterns/general/errors/validation/ (fetched 2026-09-24)
...
Help users with error recovery by accurately identifying issues and providing easy identification of incorrect fields for correction.
 ...
 [Form validation demo](https://cloudscape.design/examples/react/form-validation.html)
## Key UX concepts
...
With [form field validation](../../../components/form-field/guidelines.md) users are informed about an [error](../../../components/form-field/guidelines.md) or [warning](../../../components/form-field/guidelines.md) as soon as they leave the populated or empty field, which increases error discoverability and remediation. For example, to inform users about an incorrect character typed.

It's often used for *client-side * validation, which processes information and provides contextual feedback as [error text on individual form fields.](../../../components/form-field/guidelines.md) It's best used for validating specific values such as missing required fields, incorrect formatting of values, and unacknowledged confirmation fields. This type of validation is invoked on-blur when leaving a populated field, or an empty field. Additionally, it can be invoked on key press when fixing the value of fields in error state, when submitting a form, or transitioning to a next step in a [wizard](../../../components/wizard/guidelines.md).

#### Page level form validation

 [Page level form validation](../../../components/form/guidelines.md) is often used for *server-side * validation, which processes information *after * the form is submitted, and returns either a success or failure in one of two ways: an [alert](../../../components/alert/guidelines.md) above the form's action buttons (in the case of a recoverable failure), or a [flash](../../../components/flashbar/guidelines.md) on the page following submission (in the case of success or immutable failure). It's best used for validating requests to the server that result in recoverable errors such as insufficient capacity, exceeded request limits, and permission errors that can be updated. This type of validation is invoked upon form submission or transition to the next step in a [wizard](../../../components/wizard/guidelines.md).

[View Documentation](validation.md)

### Contextualize validation messages

Provide contextual error and warning messages below the corresponding control by using form field error and warning properties. Follow the guidance for [field validation](validation.md).
...
If contextualizing validation on a field isn't possible, such as due to a technical constraint (for example: no user permission), display a descriptive error message in an alert, provided by errorText in [form component](../../../components/form/guidelines.md).

### Make errors discoverable

#### Required fields

In [medium to complex forms,](../../resource-management/create.md) it is not uncommon for a user to overlook required fields on the first pass. We recommend to use inline validation for required fields that are left empty.
...
See an interactive example of form validation in the [validation demo](https://cloudscape.design/examples/react/form-validation.html).
...
[View Documentation](../../../components/form-field/guidelines.md)
...
import { FormField, Input, Container, Header } from '@cloudscape-design/components';
import { useState } from 'react';

const Snippet = () => {
  const [value, setValue] = useState('');
  return (
    <Container header={<Header variant="h2">Resource settings</Header>}>
      <FormField
        i18nStrings={{
          errorIconAriaLabel: 'Error',
        }}
        constraintText={`Resource name must be 3 to 28 characters. Valid characters are a-z, A-Z, 0-9, and hyphens (-). Character count: ${value.length}/28.`}
        errorText="Resource name is required."
        label="Resource name"
      >
...
For detailed information about how to communicate error and success messages see [error messages](error-messages.md).

# Create instance

## Instance settings

Name The instance name is already in use. Use a different name. Instance name must be 8 to 128 characters. Valid characters are a-z, A-Z, 0-9, and hyphens (-). Description* - optional* You have reached the maximum amount of distributions you can create. Learn more about distribution limits Cancel Create instance Error, You have reached the maximum amount of distributions you can create. Learn more about distribution limits
```
import React from 'react';
...
const Snippet = () => (
  <Form
    ...
    defaultInputValue="MyInstanceName"
    validate={() => 'The instance name is already in use. Use a different name.'}
  />
);
export default Snippet;
```
...
- Use [inline error text](../../../components/form-field/guidelines.md)   if the error is related to a specific field.
- Use an [alert](../../../components/form/guidelines.md)   above the submit button to communicate server-side errors that are not related to a field.
- Follow the guidelines for [form field](../../../components/form-field/guidelines.md)   , [alert](../../../components/alert/guidelines.md)   , and [flashbar](../../../components/flashbar/guidelines.md)   errors.
...
- Don't disable the form's submission button. Use the submission button as a fallback mechanism to launch validation error messages. Unless there are prerequisite actions that need to be completed prior to the form submission, such as in [delete with additional confirmation](../../resource-management/delete/delete-with-additional-confirmation.md)   modal.
...
- Follow the writing guidelines for [form field](../../../components/form-field/guidelines.md)  .
...
[View Documentation](../../resource-management/create.md)
...
[View Documentation](../../../components/form/guidelines.md)
...
[View Documentation](error-messages.md)
