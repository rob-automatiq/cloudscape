...
> Source: https://cloudscape.design/patterns/general/filter-patterns/ (fetched 2026-09-24)
...
[View Documentation](../../components/text-filter/guidelines.md)
...
If users tend to know exactly the value or term they are looking for, use the [text filter](../../components/text-filter/guidelines.md).

If the common behavior of users is to filter a resource by only one or two properties, use the [collection select filter](../../components/collection-select-filter/guidelines.md) . For example: by "status" or "type".

For complex products with large collection of resources, use the [property filter](../../components/property-filter/guidelines.md) so that users can combine multiple properties, values, and operators.
