...
> Source: https://cloudscape.design/patterns/general/timestamps/ (fetched 2026-09-24)
...
Creation date 5 hours ago by plrs
...
export default function TimestampRelative() {
  return (
    <>
      <p>
        <Box variant="awsui-key-label">Creation date</Box>5 hours ago by <Link href="#">plrs</Link>
      </p>
      <br />
      <p>
        <Box variant="awsui-key-label">Scan completed</Box>6 days ago
      </p>
    </>
  );
}
...
Creation date October 3, 2012, 09:32 (UTC+3:30) by plrs

Bucket deleted January 31, 2010, 14:56 (UTC+3:30) by plrs

```
import React from 'react';
import { Box, Link } from '@cloudscape-design/components';

export default function TimestampAbsolute() {
  return (
    <>
      <p>
        <Box variant="awsui-key-label">Creation date</Box>
        October 3, 2012, 09:32 (UTC+3:30) by <Link href="#">plrs</Link>
      </p>
      <br />
      <p>
        <Box variant="awsui-key-label">Bucket deleted</Box>
        January 31, 2010, 14:56 (UTC+3:30) by <Link href="#">plrs</Link>
      </p>
    </>
  );
}
...
export default function TimestampISO() {
  return <code>&#123; "creationDate": "2010-01-19T14:32:35+03:00" &#125;</code>;
}
```
...
Timestamps should always be accompanied by a label that clearly and consistently describes what event the timestamp references. You can display a timestamp label with the timestamp, above it as a heading (such as the *key* in a [key-value pair](../../components/key-value-pairs/guidelines.md) ), or as a column header in a [table](../../components/table/guidelines.md).
