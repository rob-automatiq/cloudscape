...
> Source: https://cloudscape.design/patterns/general/unsaved-changes/ (fetched 2026-09-24)

Communicate to users that unsaved changes on the page will be discarded when users leave a page.

 [View demo](https://cloudscape.design/examples/react/form-unsaved-changes.html)
## Key UX concepts
...
Show an in-page [modal](../../components/modal/guidelines.md) when users trigger the buttons and links on the page to perform actions that will result in data loss. For example, a user has made changes on a form and attempts to exit the form by:
...
import { Box, Button, Alert, SpaceBetween, Modal } from '@cloudscape-design/components';

const UnsavedChangesExample = () => {
...
export default UnsavedChangesExample;
```

#### Features

1. **Modal: **   Follow the guidelines for [modal](../../components/modal/guidelines.md)  .
2. **Alert: **   Use a warning [alert](../../components/alert/guidelines.md)   to warn users of the potential data loss.
...
- Follow the accessibility guidelines for [modal](../../components/modal/guidelines.md)   and [alert](../../components/alert/guidelines.md)  .
...
[View Documentation](../resource-management/edit.md)
