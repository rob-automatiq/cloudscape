...
> Source: https://cloudscape.design/patterns/resource-management/delete/delete-with-additional-confirmation/ (fetched 2026-09-24)
...
The delete with additional confirmation pattern helps prevent users from performing accidental, high-severity deletions by adding friction during the deletion process.
 ...
 [View demo](https://cloudscape.design/examples/react/delete-with-additional-confirmation.html)
## Building blocks
...
Make sure the user understands everything that is being deleted, and that the action is irreversible. Use important details associated with the resource being deleted, such as the resource name or other unique identifiers. Use [bold font weight](../../../foundation/visual-foundation/typography.md) to expose the resource identifier. For example: *Permanently delete instance * *CMRVMLFVHDVHU0* *? You can't undo this action.*

#### C. Consequences

State the possible consequences of the action like severity, outcome, and potential cascading effects of the action. This helps the user be aware of the actual impact and scope of the destructive action. Use the [warning alert](../../../components/alert/guidelines.md) format to present this information. If possible, add an [external link](../../../components/link/guidelines.md) to detailed documentation for users to consult.
...
Give users clear guidance on prerequisites and recommend best practices. For example, prompting users to disable the resource prior to deletion or create a snapshot to help recover from a deletion in the future. Any [links](../../../components/link/guidelines.md) leading to additional information on the prerequisites, or to prerequisite actions outside of the modal, should open in a new tab. Use the [Box](../../../components/box/guidelines.md) component with the `variant` property set to `"small"` to show the approximate deletion time if the deletion is not instantaneous.

#### G. Confirmation text input field

If there is a single action that needs to be completed prior to deletion, disable the [input field](../../../components/input/guidelines.md) to focus the user's attention on that prerequisite task.

#### H. Buttons

If there is a single action that needs to be completed prior to deletion, disable the [primary button](../../../components/button/guidelines.md) in order to focus the user's attention on that prerequisite task.
...
Make sure the user understands everything that is being deleted, and that the action is irreversible. Show the number of resources that were chosen to be deleted. Use [bold font weight](../../../foundation/visual-foundation/typography.md) to expose number and resource type. For example:* Permanently delete * *7** * *distributions? * *You can't undo this action.*

#### C. Consequences

State the possible consequences of the action like severity, outcome, and potential cascading effects of the action. This helps the user be aware of the actual impact and scope of the destructive action. Use [warning alert](../../../components/alert/guidelines.md) format to present this information. If possible, add an [external link](../../../components/link/guidelines.md) to detailed documentation for users to consult.
...
[View Documentation](delete-with-simple-confirmation.md)
...
[View Documentation](one-click-delete.md)
...
[View Documentation](../../../components/modal/guidelines.md)
