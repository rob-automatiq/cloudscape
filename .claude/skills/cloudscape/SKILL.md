...
- **Routing: `<HashRouter>` from `react-router-dom`, always.**
  - Every route lives after `#`, as in `#/customers/abc`.
...
To start a new app, copy `assets/app-template/` into an empty project and run `npm install`, then set `APP_NAME` in `App.jsx` and the `<title>` in `index.html` to the app's name. The template's Items pages are a placeholder entity: replace them with the app's own entities. For an existing project with a different structure, ask before restructuring it. Explain that this architecture is what makes routing, data and publishing work reliably in the Claude Code browser.
