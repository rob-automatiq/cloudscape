...
> Source: https://cloudscape.design/components/checkbox/ (fetched 2026-09-24)

Checkboxes enable users to turn an option on or off.

 [Get design library](../../get-started/for-designers/design-resources.md) [Browse code](https://github.com/cloudscape-design/components/tree/main/src/checkbox)
...
The checkbox component is controlled. Set the `checked` property and the `onChange` listener to store its value in the state of your application. Learn more about the [state management](../../get-started/dev-guides/state-management.md) of Cloudscape components.
...
- If only one option can be selected from a group (mutually exclusive selection), use a [radio group](../radio-group/guidelines.md)  .
- If the group of non-mutually exclusive options is higher than seven, use [multiselect](../multiselect/guidelines.md)  .
- Use for options that turn a group of elements on or off, for example progressive disclosure of form elements. If the group of sub-elements contains other checkboxes, use [toggle](../toggle/guidelines.md)   , a [radio group](../radio-group/guidelines.md)   , or [tiles](../tiles/guidelines.md)  .
- Use where a description of the selected (on/activated/enabled) state is sufficient to understand the implications of the option. Use [tiles](../tiles/guidelines.md)   or a [radio group](../radio-group/guidelines.md)   if you need to explicitly describe both the *on*   and *off*   states, for example to describe cost or performance implications.
...
- Don't use for options that require descriptions to understand the implications of both the *on*   and *off*   states. Use a [radio group](../radio-group/guidelines.md)   or [tiles](../tiles/guidelines.md)   instead.
...
- In the rare case where you need users to select a checkbox to agree to the terms of service, use the first person.  
  ...
  - See a full example of a [terms and conditions form field](../form-field/guidelines.md)    .
...
- Checkbox labels and descriptions are part of the clickable/focusable area of the control, so they should not contain interactive content (for example, links). Place links at the [form field](../form-field/guidelines.md)   level instead.
- Use the [form field component](../form-field/guidelines.md)   to group multiple checkboxes under a single label.
