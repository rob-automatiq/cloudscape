...
> Source: https://cloudscape.design/components/link/ (fetched 2026-09-24)
...
A link component is an anchor tag that defines a hyperlink, which a user can interact with to find out more information about a concept, task, or field.
 ...
 [Get design library](../../get-started/for-designers/design-resources.md) [Browse code](https://github.com/cloudscape-design/components/tree/main/src/link) If you are new to test utility classes, you can learn more in the [introduction article](../../get-started/testing/introduction.md).
...
- If navigating away would result in loss of unsaved input, use the [unsaved changes](../../patterns/general/unsaved-changes.md)   dialogue instead of opening a new tab.
...
- Don't use a link for actions. Instead, use a [button](../button/guidelines.md)  .
...
#### Normal links
  ...
  - A **primary**     link is underlined to provide more visual contrast with surrounding context. Use the `primary`     variant for a link adjacent to other text, such as inside body text paragraph or a component description, a learn more link in a [container header](../header/guidelines.md)     or [form field](../form-field/guidelines.md)     description, a item ID link in a [table](../table/guidelines.md)     , a link in a [key-value pair](../key-value-pairs/guidelines.md)     , a link in an [alert](../alert/guidelines.md)    .
  - A **secondary**     link uses regular font weight. Use the `secondary`     variant where its context implies interactivity and users can identify links from other elements in its vicinity easily, such as a list of links in a [container](../container/guidelines.md)    .

#### Info link

  An info link is bold and smaller in size. An info link is technically a button because it doesn't navigate the user to another page. Instead, it invokes the [help panel](../help-panel/guidelines.md)   to display contextual help related to an element or section of the page. Use the `info`   variant only for a link that triggers the help panel.  

  Follow the guidelines for how to use info links in [help system](../../patterns/general/help-system.md)  .
- #### External link - optional
  ...
  - For example: Learn more about [links in [another site]](https://cloudscape.design/components/link/)    .
...
- #### Size - optional
  ...
  The size sets the font size and line height of the link. The options are any of the standard [typography](../../foundation/visual-foundation/typography.md)   styles.  
...
- #### Color - optional
  ...
  The color controls the link text color. By default, all links should use the default link color. For links on inverted backgrounds such as [flashbars](../flashbar/guidelines.md)   , use `color="inverted"`  .
