...
Generated from `@cloudscape-design/components@3.0.1382` `select/interfaces.d.ts`. Descriptions are the library's own JSDoc (the same text as the API tab on cloudscape.design).
...
- **`options?`**: `SelectProps.Options`
  ...
  - `iconName` (string) - (Optional) Specifies the name of an [icon](../icon/guidelines.md) to display in the option or group.
...
- **`renderHighlightedAriaLive?`**: `SelectProps.ContainingOptionAndGroupString`
  ...
  [accessibility guidelines](../select/guidelines.md#accessibility-guidelines).
...
Supporting types from `SelectProps` (verbatim):
