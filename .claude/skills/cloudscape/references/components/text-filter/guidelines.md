...
> Source: https://cloudscape.design/components/text-filter/ (fetched 2026-09-24)

With a text filter, users can enter text that's used to match specific items in a collection.

 [Get design library](../../get-started/for-designers/design-resources.md) [Browse code](https://github.com/cloudscape-design/components/tree/main/src/text-filter)
...
This component does not perform filtering operations, only shows the UI. If you want to perform filtering operations on your data, consider using [collection hooks](../../get-started/dev-guides/collection-hooks.md).

#### State management

The text filter component is controlled. Set the `filteringText` property and the `onChange` listener to store its value in the state of your application. Learn more about the [state management](../../get-started/dev-guides/state-management.md) of Cloudscape components.
...
| --- | --- | --- | --- |
| findInput | [InputWrapper](../input/guidelines.md) | - | - |
...
- Use the text filter to provide basic filtering in a collection. The most common use cases are [table](../table/guidelines.md)   and [card](../cards/guidelines.md)   collections.
...
- Don't use a text filter for collections that have a large set of values; use the [property filter](../property-filter/guidelines.md)   instead.
- Don't use a text filter in combination with the [property filter](../property-filter/guidelines.md)   pattern. Use only the property filter.
