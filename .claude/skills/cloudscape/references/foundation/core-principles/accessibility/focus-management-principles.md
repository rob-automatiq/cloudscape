...
> Source: https://cloudscape.design/foundation/core-principles/accessibility/focus-management-principles/ (fetched 2026-09-24)
...
- **Reduce key presses**   Focus management is used to reduce the user's need to tab through content they do not wish to hear the entirety of. For example, in the [tabs component](../../../components/tabs/guidelines.md)   , a tab list is one tab stop, and the user can use arrow keys to move between the tabs, or press tab to skip the tab list.
...
When a dialog opens, Cloudscape moves focus into the dialog element. The user hears the dialog's accessible name. The Modal and Dialog components derive this name from the contents of the **"header" prop** and associate it with the dialog element by using `aria-labelledby`.

Focus behavior depends on whether the dialog is modal or non-modal.

#### Modal dialogs

The [modal](../../../components/modal/guidelines.md) component moves focus by default to the close button because it is the first interactive element. Elements outside the modal are hidden from assistive technology, and focus remains trapped within the modal. This prevents the user from moving focus behind the open modal.

#### Non-modal dialogs

The [dialog](../../../components/dialog/guidelines.md) component moves focus to the first focusable element. This is usually the first header action when provided, or the close button. Elements outside the dialog remain available to assistive technology, and the dialog doesn't trap focus. The user can move focus into and out of the dialog according to the document's focus order.

The Modal and Dialog components in Cloudscape handle the focus requirements for opening a dialog.

#### Closing a dialog

When a dialog closes, return focus to the element that had focus before the dialog opened. If that element no longer exists, move focus programmatically to a logical location. The next focusable element in the document order is usually the best option.

- **What Cloudscape handles:**  
  The Modal and Dialog components automatically return focus to the previously focused element if it still exists.
- **Implementation considerations:**  
  Move focus programmatically if the previously focused element no longer exists. This situation can occur when a dialog opens programmatically or when the element is removed while the dialog is open.

### Alerts and flashbars

It is important to make the user aware of a message in an alert or flashbar so that they are able to easily act on it. [Alerts](../../../components/alert/guidelines.md) and [flashbars](../../../components/flashbar/guidelines.md) in Cloudscape persist to ensure the user has ample time to interact with them, and move focus to make sure the user is aware of the alert. It is recommended to not rely solely on aria-live for important announcements as users who may be highly-zoomed do not have access to live announcements.
...
- **Info, in Progress, progress bar**  
  Do not move focus, use a [live region component](../../../components/live-region/guidelines.md)   to announce the message.
...
The [attribute editor](../../../components/attribute-editor/guidelines.md) in Cloudscape is an illustrative example of using focus to show outcome, and what to do when a button disappears.
...
[View Documentation](../../../components/attribute-editor/guidelines.md)
