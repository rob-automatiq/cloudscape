...
> Source: https://cloudscape.design/foundation/visual-foundation/data-vis-colors/ (fetched 2026-09-24)
...
The color palette for data that represents status or severity, and made to complement the status-related components such as [flashbars](../../components/flashbar/guidelines.md) and [status indicators](../../components/status-indicator/guidelines.md).

When applying the palette, pick the color values that represents the data best. Not all 7 values need to be used. It can be used to represent a specific set of status categories that are independent to each other, or to represent an ordered sequence of severity levels. The color tokens in this palette can be [themed](theming.md) and are marked as themeable in the table below.
...
Error bars communicate uncertainty or variability in the data and should be visually distinct without overpowering the primary chart elements. Use the following color palette for your chart series to ensure error bars are clear and consistent across different chart types and color palettes. Visit [cartesian charts](../../components/cartesian-chart/guidelines.md) for more information on error bars.
...
The color tokens in this palette cannot be [themed](theming.md).
...
- Place charts only on the default [container](../../components/container/guidelines.md)   background color for both light and dark mode.
...
- Don't use colors specific to data visualization for UI elements. For those purposes, follow the guidelines for [colors foundation](colors.md)  .
...
Whenever possible, make sure to use the design tokens for data visualization that are listed in this article. Follow the general and development [guidelines of design tokens](design-tokens.md).

If you cannot use design tokens, be aware that different [visual modes](visual-modes.md) such as dark mode will affect how your colors are perceived by users.
