...
> Source: https://cloudscape.design/patterns/general/errors/ (fetched 2026-09-24)
...
Client or server side errors that occur as a result of validation on a form, field, or flow. They usually require a user to perform an action on that field, form, or another page to remediate the error. Refer to the [validation](errors/validation.md) pattern for details on scenarios and guidelines on addressing them.

### Component specific functional errors

Errors that occur in a component due to fetching or loading issues. For example:

- The [autosuggest](../../components/autosuggest/guidelines.md)   has a built-in error state that is displayed when an error occurs while getting suggestions based on the user's entered query.
- The [multiselect](../../components/multiselect/guidelines.md)   component error state occurs when it fails at fetching options (for example, if the API fails to load the next set of options in the list).
- The [code editor](../../components/code-editor/guidelines.md)   error state is displayed when the component fails to load.
- The individual [charts](../../components/bar-chart/guidelines.md)   components have an error state that is displayed when they fail to fetch data.
...
- For form or field validation, follow the pattern recommendations outlines in [validation](errors/validation.md)  .
- For table errors, follow the guidelines for [table view](../resource-management/view/table-view.md)  .
- For errors when loading and refreshing, follow the guidelines for [loading and refreshing](loading-and-refreshing.md)  .
