...
> Source: https://cloudscape.design/patterns/general/filter-patterns/filter-persistence-in-collection-views/ (fetched 2026-09-24)
...
Ensure that components consuming data from the URL remain controlled, and treat the URL as the single source of truth for data. See example implementation in [saved filter sets](https://cloudscape.design/examples/react/table-saved-filters.html) and [property filter](https://cloudscape.design/examples/react/table-property-filter.html) demos. Note that these examples only include persistence for property filtering.
...
- Use when it is clear which collection view is persisting filters into the URL. For example, when a [table view](../../resource-management/view/table-view.md)   is the only collection on a page.
...
[View Documentation](saved-filter-sets.md)
