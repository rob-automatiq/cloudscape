...
> Source: https://cloudscape.design/patterns/resource-management/view/table-with-nested-resources/ (fetched 2026-09-24)

Use a table with nested resources when a data set contains resources organized into parent-child relationships.

 [View playground example](../../../components/table/guidelines.md) [View demo](https://cloudscape.design/examples/react/table-expandable.html)
## Key UX concepts
...
Nested resources can be visualized using a [table with expandable rows](../../../components/table/guidelines.md) . Use this pattern when data has multiple levels of hierarchy and it is important to preserve in-page context while comparing or interacting with multiple items.

Deep or highly complex nesting can negatively affect performance and readability. In such cases, consider alternative approaches, such as using a [details page](../details/details-page.md) or splitting the data across multiple tables.

If your data does not represent parent-child relationships, but instead consists of resources **grouped by shared characteristics** , consider using the [table with grouped resources](table-with-grouped-resources.md) pattern instead.
...
In tables with nested resources, each resource row must be individually selectable. Selecting all related resources (for example, selecting all children of a parent or all descendants) should be performed through [in-context table actions](../../../components/table/guidelines.md).
...
When selecting a parent resource implicitly affects its children, clearly communicate this impact. Use a [confirmation modal](../delete/delete-with-simple-confirmation.md) to indicate the scope of the action, for example: "Delete `B` and its child resources `X` , `Y` , and `Z` ? This action cannot be undone."
...
[View Documentation](../../../components/pagination/guidelines.md)
...
Use the table component for this pattern. See the [table view](table-view.md) pattern for more details about tables in general.
...
If selecting and performing an action on the parent will also affect its children, indicate this with a [confirmation modal](../delete/delete-with-simple-confirmation.md) when taking an action.

## General guidelines

### Don't

- Don't use a table with nested resources for extremely large or deeply complex data sets. Instead, consider using a [details page as a hub](../details/details-page-as-hub.md)   or splitting the data across multiple tables.
